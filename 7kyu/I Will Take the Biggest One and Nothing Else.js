...
const maxIntChain = (n) => {
  ...
  for (let c = 1; c < n / 2; c++) {
    let d = n - c;
    if (c * d > largestN1 && c !== d) largestN1 = c * d;
  }
...
// Another good user solution:
const maxIntChain = (n) => (n < 5 ? -1 : Math.floor(n / 2 + 1) * Math.ceil(n / 2 - 1));

/* Takeaways:
1) Not sure! Need to understand better why the users' formula works.
I'd imagine because to maximize N1, we want values closest to the square root of n. Since the Kata specifies that a !== b (and thus not the actual square root of n), this leaves us with ceil((n/2)^2 - 1) * floor((n/2)^2 + 1) as the maximum N1.
*/
