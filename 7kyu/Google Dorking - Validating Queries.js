...
// My solution:
const isValid = (query) => query.match(/\w+(?=:)/g).every((q) => FILTERS.includes(q));

// An alternative user solution:
const isValid = (query) => {
  return query
    .split(' ')
    .filter((queryChunk) => queryChunk.indexOf(':') > -1)
    .every((filteredQueryChunk) => FILTERS.some((filter) => filter === filteredQueryChunk.split(':')[0]));
};
