...
const oddOrEven = (n, half = n / 2) => (!Number.isInteger(half) ? EITHER : half % 2 ? ODD : EVEN);

// My solution:
const oddOrEven = (n) => {
  const halfN = n / 2;
  if (!Number.isInteger(halfN)) return EITHER;
  if (halfN % 2) return ODD;
  return EVEN;
};
...
/* Takeaways:
1) My solution contained superfluous logic. Rather than testing if half of n is an integer or not, we could have just tested if n was odd!
*/
