...
const getNewNotes = (salary, bills) => {
  const disposable = salary - bills.reduce((sum, curr) => sum + curr, 0);
  return disposable <= 0 ? 0 : Math.floor(disposable / 5);
};

// Top user solution:
const getNewNotes = (salary, bills) =>
  (Math.max(0, salary - bills.reduce((pre, val) => pre + val, 0)) / 5) | 0;
