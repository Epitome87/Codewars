...
const getRequired = (player, enemy) => {
  const pScore = player[0] + player[1];
  const eScore = enemy[0] + enemy[1];
  if (pScore === eScore) return 'Random';
  if (pScore >= eScore + 6) return 'Auto-win';
  if (eScore >= pScore + 6) return 'Auto-lose';
  if (pScore > eScore) return `(${eScore + 7 - pScore}..6)`;
  if (eScore < pScore + 5) return `(1..${pScore + 5 - eScore})`;
