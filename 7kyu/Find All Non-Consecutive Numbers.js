...
const allNonConsecutive = (arr) =>
  arr.reduce((acc, curr, idx, a) => (idx > 1 && curr !== a[idx - 1] + 1 ? [...acc, { i: idx, n: curr }] : acc), []);

// Could also ignore the index check by splicing and altering some logic:
const allNonConsecutive = (arr) =>
  arr.slice(1).reduce((acc, curr, idx) => (curr !== arr[idx] + 1 ? [...acc, { i: idx + 1, n: curr }] : acc), []);
