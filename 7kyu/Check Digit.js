...
// My (ugly) one-liner:
const checkDigit = (num, idx1, idx2, digit) =>
  `${num}`.slice(Math.min(idx1, idx2), Math.max(idx1, idx2) + 1).includes(digit);

// My other solution:
const checkDigit = (num, idx1, idx2, digit) => {
  const minIndex = Math.min(idx1, idx2);
  const maxIndex = Math.max(idx1, idx2);
  const numAsStr = num.toString().slice(minIndex, maxIndex + 1);

  return numAsStr.includes(digit);
};
...
const checkDigit = (num, idx1, idx2, digit) => {
  const minIndex = Math.min(idx1, idx2);
  const maxIndex = Math.max(idx1, idx2);
  const numAsStr = num.toString().slice;

  for (let i = minIndex; i <= maxIndex; i++) {
    if (+numAsStr[i] === digit) return true;
  }
