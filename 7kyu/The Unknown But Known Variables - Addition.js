...
const theVar = (theVariables) => {
  const [varA, varB] = theVariables.split('+');
  return varA.charCodeAt() - 96 + varB.charCodeAt() - 96;
};
...
const theVar = (theVariables) => {
  ...
  const [varA, varB] = theVariables.split('+');
  return map.indexOf(varA) + map.indexOf(varB);
};

// Top user solution:
const theVar = (theVariables) => theVariables.charCodeAt() + theVariables.charCodeAt(2) - 192;
