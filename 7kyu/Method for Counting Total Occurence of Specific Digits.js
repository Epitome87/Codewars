...
function List() {
  this.countSpecDigits = (intList, digitList) =>
    digitList.map((digit) => [digit, intList.join('').split(digit).length - 1]);
}
...
function List() {
  this.countSpecDigits = function (integersList, digitsList) {
    ...
    for (let digit of digitsList) {
      let count = 0;
      for (let int of integersList) {
        const intArr = [...int.toString()];
        count += intArr.reduce((acc, curr) => (+curr === digit ? acc + 1 : acc), 0);
      }
