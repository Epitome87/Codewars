...
We cannot endanger the lives of our passengers, so if we get erroneous inputs, our elevator must remain on the same level. So for example:
...
const goto = (level, button) => {
  if (typeof level !== 'number' || typeof button !== 'string' || /[^0-3]/.test(level) || /[^0-3]/.test(button))
    return 0;
  return +button - +level;
};
