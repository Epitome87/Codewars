...
const race = (v1, v2, g) => {
  ...
  const secondsToCatchup = (3600 * -g) / (v1 - v2);
  const hours = (secondsToCatchup / 3600) | 0;
  const minutes = ((secondsToCatchup % 3600) / 60) | 0;
  const seconds = (secondsToCatchup % 3600) % 60 | 0;
...
// User solution similar to mine (slightly different time logic):
function race(v1, v2, g) {
  if (v2 < v1) {
    return null;
  }
